...
技术栈：Astro、TypeScript、Content Collections、Markdown/MDX、原生 CSS、内联 SVG、少量浏览器 TypeScript、Nginx 静态部署。没有数据库、CMS、用户系统或应用后端。
...
- `scripts/verify.mjs`：顺序执行 lint、Astro check 和 build
...
`verify` 依次运行 lint、Astro check 和 build；任一步失败即停止。Windows Git Bash 若无法直接调用 pnpm，可运行 `node scripts/verify.mjs`。
