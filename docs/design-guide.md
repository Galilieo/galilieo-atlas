...
文章页的返回入口位于封面之前，使用独立的原生 sticky 导航停靠在顶部导航下方（距视口顶部 88px），与阅读区左边缘对齐。按钮为 40px 圆角方形，使用 20px 线条 SVG 左箭头，不显示文字或内层圆底；保留“返回博客”的无障碍名称与悬停提示。不要把它放回带 Reveal 或裁切的封面内部；按钮使用实色主题底色，空白区域不拦截点击。正文 H2/H3 为返回入口预留锚点滚动间距，无 JavaScript 时仍可返回博客列表。

首页个人卡在 580px 以下将头像与名称并排，身份说明和简介跨满卡片宽度，顶部为完整档案入口预留空间。标题与简介使用自然换行；个人卡仅在精细指针悬停或键盘聚焦主入口时轻微上移，不缩放卡片或旋转头像，Reduced Motion 下保持静止。
