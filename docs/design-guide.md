...
- GitHub 功能卡只把官方 Public Events 表达为“最近 30 天公开活动”，不得冒充年度 Contributions；使用 30 分钟缓存，并保留 API 限流/离线回退。
...
- 首页个人档案卡使用兄弟覆盖层导航：头像、简介、留白和“查看完整档案”进入 `/about/`；Works / Notes 分别进入 `/projects/`、`/blog/`；GitHub、邮件和 Heart Island 保留独立目标。所有内部入口位于整卡覆盖层之上，不嵌套 `<a>`。项目与笔记轮播同样使用两层兄弟链接：点击外层标题、留白或边缘分别进入 `/projects/`、`/blog/`，点击内层当前视觉卡进入对应项目或文章详情；内层不增加“查看项目 / 阅读笔记”等解释文字。轮播圆点是更高层的独立切换控件，不参与导航。
...
博客列表从公开文章的多值 `tags` 聚合筛选项与数量，`/blog/` 和 `/notes/` 必须复用同一筛选组件。筛选为单选渐进增强：按钮使用 `aria-pressed`，结果状态通过 `aria-live` 更新；Reduced Motion 不依赖动画表达状态，无 JavaScript 时隐藏无效筛选条并展示全部文章。主分类 `category` 与多值 Tag 的职责不可混用。
...
- 当前 Demo 临时使用 `src/assets/images/home/demo-background-placeholder.webp` 作为低透明度整页氛围层，并由主题遮罩保证内容可读。它由站点所有者从桌面文件 `【哲风壁纸】二次元-动漫-城市.png` 提供，仓库内仅保存压缩后的 2560×1440 WebP；原作者、授权范围和正式发布用途仍待确认，因此正式上线前必须完成授权确认或替换。最终背景不得据此重排首页。
